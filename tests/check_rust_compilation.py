...
logger.info("AuroraView module: %s" % module_path)

# Get the directory containing the compiled .pyd/.so files
module_dir = os.path.dirname(module_path)
logger.info("Module directory: %s" % module_dir)
...
if not compiled_files:
    ...
    logger.info("\nFound %d compiled Rust file(s):" % len(compiled_files))
    ...
    for filepath in compiled_files:
        ...
        logger.info("  Modified: %s" % mtime_str)
        logger.info("  Size: %.2f MB" % size_mb)
        
        if age_hours < 1:
            logger.info("  Age: %.1f minutes (RECENT)" % age_minutes)
        elif age_hours < 24:
            logger.info("  Age: %.1f hours" % age_hours)
        else:
            age_days = age_hours / 24
            logger.info("  Age: %.1f days (OLD)" % age_days)
...
if compiled_files:
    ...
    if age_hours < 1:
        ...
        logger.warning("WARNING: Rust code was compiled %.1f hours ago" % age_hours)
